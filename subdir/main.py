''' 12 coins problem:  you are given 12 coins that look identical and a pair of scales.
    One of the coins is fake and has a different weight than the others.  Your challenge 
    is to use the scales at most three times to determine the fake coin and whether it 
    is light or heavier than a real coin.  You can weigh any group of coins against 
    another using the scales to see which is heavier or if they balance, but you cannot 
    measure the actual weights of the coins. '''
...
''' The code we are constructing uses the alphabet { O, R, L } with each word being 
    ...
    3) For each position 1-3, there are equal numbers of L's and R's.
    
    
    Any such code determines a method of placing coins on the scales in the three 
    weighings: we assign a coin to each codeword, e.g. coin #1 assigned to ORR.  
